...
int sendFile(int sockfd, struct sockaddr * serv_addr){
 ...
 while (1) {
	...
	printf("Remaining data = %d\n", remain_data);
        ...
        if (sendto(sockfd, buffer, read_return, 0, serv_addr, sizeof(*serv_addr)) == -1) {
            perror("write");
            exit(EXIT_FAILURE);
        }
}
...
*/
    ...
    while (recvfrom(socketDscrp,msg,sizeof(msg),0,(struct sockaddr *)&serv_addr,sizeof(serv_addr))==0);
