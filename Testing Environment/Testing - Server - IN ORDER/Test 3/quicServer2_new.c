...
int main(){
	int socketDscrp= socket(AF_INET, SOCK_DGRAM, IPPROTO_QUIC);
	if(socketDscrp<0){
		printf("XXXXXXXX	QUIC Socket creation failed	XXXXXXXX\n");
		exit(1);
	}
	else{
		printf("******	QUIC Socket created with Descriptor: %d\n", socketDscrp);
	}
	printf("\nSocket in operation\n\n");
